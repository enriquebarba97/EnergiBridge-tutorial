...
# Optional friendly names for each configuration. Keep this aligned with COMMANDS.
# Leave entries empty to fall back to autogenerated names.
CONFIG_NAMES=(
	"alpine"
	"ubuntu"
)
...
if [[ ${#CONFIG_NAMES[@]} -gt 0 ]] && [[ ${#CONFIG_NAMES[@]} -ne ${#COMMANDS[@]} ]]; then
	echo "Error: CONFIG_NAMES must have the same number of entries as COMMANDS."
	exit 1
fi
